...
const checkIfPrerequisite = function(numCourses, prerequisites, queries) {
    const coursePrerequisite = new Map();
    ...
    const dfs = (course) => {
        if(coursePrerequisite.has(course)) {
            return coursePrerequisite.get(course);
        }

        coursePrerequisite.set(course, new Set());
        for(let prerequisite of adjList.get(course)) {
            coursePrerequisite.set(course, new Set([...coursePrerequisite.get(course), ...dfs(prerequisite)]));
        }
        coursePrerequisite.get(course).add(course);
        return coursePrerequisite.get(course);
    };
    ...
    const result = [];
    for(let [prerequisite, course] of queries) {
        result.push(coursePrerequisite.get(course).has(prerequisite));
    }
    return result;
};
